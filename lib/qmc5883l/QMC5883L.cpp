...
int16_t readBytes(uint8_t addr, unsigned char tmp)
{
	uint8_t buffer[14];
	I2Cdev::readBytes(addr, tmp, 2, buffer);
...
void QMC5883L::initialize() {
  ...
  Serial.println("QMC5883L Initializing...");
  reset();
}
...
{
  uint8_t buffer[14];
  Serial.println("QMC Ready Read Data: "+I2Cdev::readByte(addr, QMC5883L_STATUS, buffer));
